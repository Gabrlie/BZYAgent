...
## 业务目标
EduAgent Prime 面向职业院校教学场景，提供课程资产的结构化管理与 AI 文档生成能力，目标是降低教师备课与文档整理成本，提高授课计划与教案的标准化程度。
...
- AI 对话，支持流式回复与历史记录。
- 软著材料管理，支持项目配置、需求维护、后台生成与 ZIP 下载。
...
## 业务约束与规则
- 文件上传支持 `.doc` `.docx`，且单文件大小不超过 10MB。
- 授课计划与教案生成使用 SSE 传递进度，前端需处理流式状态。
- 软著材料生成采用后台任务与长轮询查询进度，允许关闭页面后继续生成。
- 授课计划生成基于课程目录，课程目录为空时禁止生成。
- 教案生成依赖授课计划文档，且仅支持系统生成的授课计划；上传授课计划不可用于教案生成与编辑。
- 授课计划命名固定为 `《课程名称》授课计划`。
...
- 授课计划排课由系统生成，需支持第一周上课次数与不上课列表配置，AI 仅生成教学内容，且可用课次可比实际课次多 0-6 次。
- Base URL 必须以 `http://` 或 `https://` 开头，并以 `/v1` 结尾。
- 软著材料输出仅 ZIP，不展示输出目录路径。
